...
def _rgb(r, g, b, a=1.0):
    # Apply alpha by scaling RGB components (alpha in 0..1)
...
class AIDOOverlay:
    """
    Full-screen transparent overlay that draws pulsing wave borders.
    """
    ...
    def hide(self):
        if self._visible:
            self._fade_dir = -1
            # _animate() will handle the fade out and cleanup automatically
    ...
    def _animate(self):
        ...
        self._job = self.master.after(interval, self._animate)
    ...
    def _draw(self):
        ...
        for i in range(n):
            ...
            for s in range(seg + 1):
                ...
                # TOP edge: y goes from 0 inward
                x = frac * sw
                y = wave + amp * 0.3
                pts_top.extend([x, max(0, y)])

                # BOTTOM edge: y goes from sh inward
                x = frac * sw
                y = sh - (wave2 + amp * 0.3)
                pts_bottom.extend([x, min(sh, y)])

                # LEFT edge: x goes from 0 inward
                y2 = frac * sh
                x2 = amp2 * math.sin(frac * math.pi * 4 + phase + 1.0) + amp2 * 0.3
                pts_left.extend([max(0, x2), y2])

                # RIGHT edge: x goes from sw inward
                y2 = frac * sh
                x2 = sw - (amp2 * math.sin(frac * math.pi * 4 + phase + 1.0) + amp2 * 0.3)
                pts_right.extend([min(sw, x2), y2])
        ...
        corners = [(0, 0), (self._sw, 0), (0, self._sh), (self._sw, self._sh)]
        ...
        # HUD label
        pulse_txt = 0.5 + 0.5 * math.sin(t * 3)
        txt_col   = _rgb(*c1, self._alpha * pulse_txt)
        self.canvas.create_text(
            self._sw // 2, 22,
            text="◈  A.I.D.O  CONTROL  MODE  ACTIVE  ◈",
            fill=txt_col, font=("Courier New", 11, "bold")
        )

        # Bottom status line
        self.canvas.create_text(
            self._sw // 2, self._sh - 18,
            text="SAY  'RELEASE'  TO  RETURN  CONTROL",
            fill=_rgb(*c2, self._alpha * 0.6),
            font=("Courier New", 9)
        )
